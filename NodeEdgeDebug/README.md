...
[`selenium/selenium-node-edge-debug` Dockerfile](Dockerfile)
...
```
$ docker run -d -p 5900:5900 --link selenium-hub:hub -v /dev/shm:/dev/shm selenium/selenium-node-edge-debug
```
...
``` dockerfile
FROM selenium/selenium-node-edge-debug:3.141.59-20210929
