...
[`selenium/selenium-node-chrome-debug` Dockerfile](Dockerfile)
...
```
$ docker run -d -p 5900:5900 --link selenium-hub:hub -v /dev/shm:/dev/shm selenium/selenium-node-chrome-debug
```
...
``` dockerfile
FROM selenium/selenium-node-chrome-debug:3.141.59-20210929
